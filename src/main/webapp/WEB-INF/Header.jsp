        ...
        <div class="header-container"
            ...
            <div class="header-search" style="flex:1 1 auto;min-width:0;max-width:400px;margin:0;position:relative;">
                <i class="fa fa-search"
                    style="position:absolute;left:14px;top:50%;transform:translateY(-50%);color:#7A5533;font-size:14px;"></i>
    ...
    (function () {
        ...
        function performSearch(q) {
            ...
            if (filtered.length === 0) {
                res.html('<div style="padding:10px; color:#7A5533; font-size:13px; text-align:center;">No users found</div>');
            } else {
                let html = '';
                filtered.forEach(u => {
                    let haspic = u.picture && u.picture.trim() !== '' && u.picture !== 'null';
                    let username = escapeHtml(u.username);
                    let firstName = escapeHtml(u.firstName);
                    let lastName = escapeHtml(u.lastName);
                    let picture = escapeHtml(u.picture);
                    let uid = encodeURIComponent(u.uid);
                    let avatar = haspic
                        ? '<img src="' + picture + '" style="width:32px;height:32px;border-radius:50%;object-fit:cover;flex-shrink:0;">'
                    ...
                    html += '<a href="#" class="search-result-user" data-uid="' + uid + '" '
                        ...
                        + avatar
                        + '<div>'
                        + '<div style="font-weight:700;color:#46331F;font-size:13px;">' + firstName + ' ' + lastName + '</div>'
                        + '<div style="color:#7A5533;font-size:11px;">@' + username + '</div>'
                        + '</div></a>';
                });
                res.html(html);
            }
        ...
        $('#searchInput').off('focus input').on('focus input', function () {
        ...
        $(document).off('click.headerSearch').on('click.headerSearch', function (e) {
            if (!$(e.target).closest('#searchInput, #searchResults').length) {
                $('#searchResults').hide();
            }
        });

        $(document).off('click.headerSearchResult').on('click.headerSearchResult', '.search-result-user', function (e) {
            e.preventDefault();
